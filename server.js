...
if (cluster.isMaster) {
  ...
  app.get("/stream/:videoId", async (req, res) => {
    try {
      console.log("serving music from worker: ", clster.worker.id);
  ...
  io.sockets.on("connection", function (socket) {
    ...
    socket.on("play next", function (data) {
      ...
      if (io.sockets.adapter.rooms["room-" + socket.roomnum] !== undefined) {
        if (
          io.sockets.adapter.rooms["room-" + socket.roomnum].queue.yt.length > 0
        ) {
          // Gets the video id from the room object
          nextObj =
            io.sockets.adapter.rooms["room-" + socket.roomnum].queue.yt.shift();
        }
  ...
  http.listen(process.env.PORT || 9999, function () {
    console.log("----=== SERVER STARTED ===---- worker: ", cluster.worker.id, " process id: ", process.pid);
  });
}
